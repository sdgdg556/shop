...
<form action="order.php" method="post" name="shopcar">
    ...
    <?php foreach($rows as $row):?>
        <tr>
			...
			<td>
             ...
             <td><input type="number" name="count<?php echo $row['id']?>" value="1" min="1"/></td>
             <td><input type="button" value="删除" class="btn"onclick="delShopcarPro(<?php echo $row['id'];?>)"></td>
        </tr>
       <?php  endforeach;?>
    </tbody>
</table>
    <div class="button">
        <td><input type="image" name="submit_order" src="images/icon/submit_order.jpg" onClick="document.shopcar.submit()"></td>   
    </div>
</form>
<div class="hr_25"></div>
<div class="footer">
...
function delShopcarPro(id){
    if(window.confirm("您确认要删除这件商品吗?")){
        window.location="doAction.php?act=delShopcarPro&id="+id;
    }
}
</script>
</body>
